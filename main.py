...
output_string = (f"В исходном датасете содержалось {all_marks.size} оценок, из них {group_marks} оценок относятся к группе ПИ101\n"
f"В датасете находятся оценки {unique_student_count.size} студентов со следующими личными номерами {group_name}: {convert_series_to_string(unique_student_count)}\n"
